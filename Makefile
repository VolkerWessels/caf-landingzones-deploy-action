...
login: ## Login to azure using a service principal
	...
	if [ -v ARM_SUBSCRIPTION_ID ]; then \
		...
		ARM_SUBSCRIPTION_ID="$$(az account list --refresh --query '[? contains(state, `Enabled`) && name==`${ARM_SUBSCRIPTION_NAME}` && tenantId ==`${ARM_TENANT_ID}`].id' --output tsv --only-show-errors)"; \
		az account set --subscription "$${ARM_SUBSCRIPTION_ID}"; \
